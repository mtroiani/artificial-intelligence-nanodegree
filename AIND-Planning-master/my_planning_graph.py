...
class PlanningGraph():
    ...
    def add_literal_level(self, level):
        ...
        for act in self.a_levels[level -1]:
            ...
            for node in act.effnodes:
                ...
                #connect the nodes
                node.parents.add(act)
                act.children.add(node)
    ...
    def interference_mutex(self, node_a1: PgNode_a, node_a2: PgNode_a) -> bool:
        ...
        for effect1 in node_a2.action.effect_rem:
            for precon in node_a2.action.precond_pos:
                if effect1 == precon:
                    return True
        return False
    ...
    def competing_needs_mutex(self, node_a1: PgNode_a, node_a2: PgNode_a) -> bool:
        ...
        #Based on this forum post: https://discussions.udacity.com/t/problem-with-test-competing-needs-mutex/227344
        ...
        for goal in self.problem.goal:
            for level in range(len(self.s_levels)):
                ...
                for node in self.s_levels[level]:
                    #need to account if it is positive or negative as symbol only gives text
                    literal = node.symbol if node.is_pos else '~{}'.format(node.symbol)
                    literals.append(expr(literal))
